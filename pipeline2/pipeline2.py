...
def main():
    ...
    pipeline1 = "pipeline2-1.py -r1 " + args.input1 + " -r2 " + args.input2 + " -o " + args.output + "result/ "

    if args.bowtie:
        check_return(os.system("mkdir " + args.output + 'data/bowtie2-index'))
        check_return(
            os.system("bowtie2-build " + args.output + "data/bedtool.fa " + args.output + "result/bowtie2-index/cds"))
        pipeline1 = pipeline1 + "-b "

    if args.diamond:
        check_return(os.system("mkdir " + args.output + 'data/diamond-db'))
        check_return(os.system(
            "diamond makedb --in" + args.output + "data/bedtool.fa -d" + args.output + "result/diamond-db/cds"))  # cds.dmnd
        pipeline1 = pipeline1 + "-d "

    if args.minimap:
        check_return(os.system("mkdir " + args.output + 'data/minimap-db'))
        check_return(os.system("cp " + args.output + "data/bedtool.fa " + args.output + "data/minimap-db/cds.fa"))
        pipeline1 = pipeline1 + "-m "

    if args.bwa:
        check_return(os.system("mkdir " + args.output + 'data/bwa-db'))
        check_return(os.system(
            "cp " + args.output + "data/bedtool.fa " + args.output + "data/bwa-db/cds.fa;cd " + args.output + "data/bwa-db;bwa index cds.fa"))
        pipeline1 = pipeline1 + "-b "
